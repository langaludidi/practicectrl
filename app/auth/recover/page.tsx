...
export default async function RecoverPage({searchParams}:{searchParams:Promise<{invite?:string;reason?:string}>}){
  ...
  return <main className="mx-auto flex min-h-screen max-w-lg items-center px-5 py-12">
    <section className="w-full rounded-3xl border border-stone-200 bg-white p-7 shadow-sm">
      ...
      <p className="mt-2 text-sm text-stone-600">Request a secure email code, then choose your password and continue your staff invitation. The code works even when your email opens in another browser.</p>
      {reason==="link_unusable"?<p role="alert" className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">That email link could not complete setup. Request a new email and enter its six-digit code here. If email sending is temporarily limited, wait before requesting another.</p>:null}
